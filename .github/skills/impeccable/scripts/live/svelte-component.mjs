    ...
    const propsComment =
        contract.length > 0
            ? `\n<!-- Props: ${contract
                  ...
                  .join(", ")} -->\n`
            : "";
    return `${buildPropsScript(
        contract
    )}${propsComment}${originalWithProps.trim()}\n\n<style>\n  /* Variant ${variantNum}: add scoped CSS here */\n</style>\n`;
}

function buildInsertVariantStub(variantNum) {
    return `${buildPropsScript(
        []
    )}<div class="impeccable-insert-preview">Insert variant ${variantNum}</div>\n\n<style>\n  .impeccable-insert-preview { display: block; }\n</style>\n`;
}
...
function variantSelectorRegex(variantNum) {
    return new RegExp(
        `\\[data-impeccable-variant=(["'])${escapeRegExp(
            String(variantNum)
        )}\\1\\]`,
        "g"
    );
