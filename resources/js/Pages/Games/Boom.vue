...
function tick() {
    ...
    if (toiletX.value >= maxX) {
        toiletX.value = maxX;
        toiletDir = -1;
    }
    if (toiletX.value <= minX) {
        toiletX.value = minX;
        toiletDir = 1;
    }
...
function startDrag(e) {
    ...
    activeMoveHandler = (ev) => {
        ...
        poopX.value = Math.max(
            POOP_SIZE / 2,
            Math.min(gameW.value - POOP_SIZE / 2, pos.x)
        );
        poopY.value = Math.max(
            POOP_SIZE / 2,
            Math.min(gameH.value - TOILET_H - TOILET_BOTTOM - POOP_SIZE, pos.y)
        );
    };
    ...
    document.addEventListener("touchmove", activeMoveHandler, {
        passive: false,
    });
...
.miss-anim-enter-active {
    animation: missIn 0.3s ease-out;
}
.miss-anim-leave-active {
    animation: missOut 0.9s ease-in forwards;
}
