    ...
    >
      <div
        ...
        :style="{
          ...
          transform: `perspective(1000px) 
                     rotateX(${Math.pow(scrollProgress, 0.8) * 8}deg) 
                     rotateY(${Math.pow(scrollProgress, 0.8) * 20}deg) 
                     translateZ(${Math.pow(scrollProgress, 0.8) * 25}px)
                     translateX(${Math.pow(scrollProgress, 0.8) * 15}px)`
        }"
        role="button"
        tabindex="0"
        @click="reloadBook"
      >
        ...
        >
          <div class="flex-1 flex items-start justify-center pt-8">
            <div class="text-center">
              <h1
                class="book-title font-heading text-5xl md:text-6xl lg:text-7xl font-bold text-white mb-4 md:mb-6 tracking-[0.1em] leading-tight"
                :data-text="book.title"
                :style="{
                  transform: `translateY(${
                    Math.pow(scrollProgress, 0.8) * -10
                  }px)`,
                  opacity: 0.9 + Math.pow(scrollProgress, 0.8) * 0.1
                }"
              >
                {{ book.title }}
              </h1>
          ...
          <div class="flex justify-center items-center mb-6">
            <div
              ...
              :style="{
                transform: `translateY(${Math.pow(scrollProgress, 0.8) * 8}px)`,
                opacity: 0.9 + Math.pow(scrollProgress, 0.8) * 0.1
              }"
            >
              <p
              ...
              <span v-if="canEditPages" class="block space-y-0.5">
                ...
                <p class="text-white text-xs drop-shadow-lg">
                  Read {{ Math.round(book.read_count).toLocaleString() }} times
                </p>
              </span>
              <p
                v-if="book.category"
        ...
        <div
          class="book-spine"
          :style="{
            transform: `translateZ(${Math.pow(scrollProgress, 0.8) * 20}px)`,
            width: `${Math.pow(scrollProgress, 0.8) * 12}px`
          }"
        ></div>
...
const handleScroll = () => {
  try {
    ...
    const rect = bookCoverRef.value.getBoundingClientRect();
    ...
    if (rect.top < windowHeight && rect.bottom > 0) {
      ...
      if (rect.top > startThreshold) {
        ...
        const currentPosition = startThreshold - rect.top;
        const progress = currentPosition / animationRange;

        if (hasInitialized.value) {
          if (isScrollingUp.value) {
            scrollProgress.value = 1 - progress;
          } else {
