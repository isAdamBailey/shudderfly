...
const calculateUploadTimeout = (fileSizeBytes) => {
    ...
    const calculatedTimeout =
        (fileSizeBytes / MIN_BANDWIDTH_BYTES_PER_SEC) *
        BUFFER_MULTIPLIER *
        1000;

    return Math.max(
        MIN_TIMEOUT_MS,
        Math.min(MAX_TIMEOUT_MS, calculatedTimeout)
    );
};
    ...
    process: (
        ...
        const send = async (body, bodySource, attempt = 0) => {
            ...
            const diagnostics = () => {
                ...
                return (
                    ...
                    `elapsed=${elapsedSec}s size=${sizeMb}MB type=${
                        file.type || "?"
                    } ` +
            ...
            xhr.addEventListener("error", () => {
                ...
                if (
                    bytesSent < body.size &&
                    attempt < MAX_NETWORK_ERROR_RETRIES
                ) {
                    setTimeout(() => {
        ...
        const start = async () => {
            ...
            if (
                !body &&
                originalFile.size &&
                originalFile.size <= MAX_SNAPSHOT_BYTES
            ) {
