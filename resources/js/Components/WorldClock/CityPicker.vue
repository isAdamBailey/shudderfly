...
<template>
    <div class="rounded-lg bg-gray-800 p-4">
        ...
        <p v-if="atLimit" class="mt-2 text-xs text-amber-400">
            Maximum of {{ maxCities }} clocks reached. Remove one to add
            another.
        </p>
        ...
        <ul v-else-if="results.length" class="mt-3 space-y-1">
            <li v-for="city in results" :key="`${city.timezone}-${city.name}`">
                ...
                >
                    <span>
                        {{ city.name }}
                        <span v-if="city.country" class="text-gray-400"
                            >· {{ city.country }}</span
                        >
                    </span>
                    <i
                    ...
                    <i
                        v-else
                        class="ri-add-line text-gray-400"
                        aria-hidden="true"
                    ></i>
                </button>
            </li>
        ...
        <ul v-if="selectedCities.length" class="mt-4 space-y-2">
            ...
            >
                <span class="shrink-0 text-sm text-gray-100">{{
                    city.name
                }}</span>
                <input
                    v-if="canRelabel"
