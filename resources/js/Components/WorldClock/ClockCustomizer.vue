...
const spokenRemaining = computed(() => {
    ...
    if (mm > 0) {
        parts.push(
            `${mm} ${
                mm === 1 ? t("world_clock.minute") : t("world_clock.minutes")
            }`
        );
    }
    if (ss > 0) {
        parts.push(
            `${ss} ${
                ss === 1 ? t("world_clock.second") : t("world_clock.seconds")
            }`
        );
    }
    if (parts.length === 0) parts.push(t("world_clock.zero_seconds"));
    return t("world_clock.time_left", {
        time: parts.join(` ${t("world_clock.and")} `),
    });
});
...
const NUMERAL_OPTIONS = [
    {
        value: "arabic",
        label: "Arabic",
        speech: t("world_clock.numerals_arabic"),
    },
...
const SECOND_OPTIONS = [
    {
        value: "smooth",
        label: "Smooth",
        speech: t("world_clock.second_hand_smooth"),
    },
